...
function fastestRunners(runners) {
  ...
  const withTotalTimes = runners.map(runner => {
    const pacesInSec = runner.paces.map(toSeconds);
    const totalTime = pacesInSec.reduce((a, b) => a + b, 0);
    const avgPaceSec = Math.round(totalTime / runner.paces.length);
    const fastestPaceSec = Math.min(...pacesInSec);
    return {
      name: runner.name,
      totalTime,
      averagePace: fromSeconds(avgPaceSec),
      fastestPace: fromSeconds(fastestPaceSec)
    };
  });

  const avgTotal =
    withTotalTimes.reduce((sum, r) => sum + r.totalTime, 0) /
    withTotalTimes.length;

  const filtered = withTotalTimes
    .filter(r => r.totalTime < avgTotal)
    .sort((a, b) => a.totalTime - b.totalTime);

  return filtered.map(({ name, averagePace, fastestPace }) => ({
    name,
    averagePace,
    fastestPace
  }));
...
const result = document.getElementById('fastest');
result.innerHTML = JSON.stringify(fastestRunners(Runners));
